...
def phenotype_gid(p):
    ...
    a.append(str(p.get('id', '').replace(':', '_').encode('utf-8')))
    a.append(str(p.get('term', '').encode('utf-8')))
    return 'phenotype:' + ':'.encode('utf-8').join(a)
...
def normalize(hit):
    ...
    try:
        if 'phenotypes' in hit['association']:
            for p in hit['association']['phenotypes']:
                gid = phenotype_gid(p)
                phenotypes[gid] = bmeg_phenotype(p, gid)
            for k in phenotypes.keys():
                if str(k) in exported:
                    del phenotypes[k]
            exported.extend(phenotypes.keys())
            del hit['association']['phenotypes']
