...
def normalize_pubchem_id(id):
    ...
    compounds = []
    id = id.replace('CID', '')
    url = 'https://pubchem.ncbi.nlm.nih.gov/rest/pug/compound/cid/{}/synonyms/JSON'.format(id)  # NOQA
    r = requests.get(url, timeout=60)
    rsp = r.json()
    if 'InformationList' in rsp:
        informationList = r.json()['InformationList']
...
def normalize_pubchem_substance_id(id):
    """ call pubchem and retrieve substance and most common synonym
        see https://pubchem.ncbi.nlm.nih.gov/rdf/#_Toc421254632
    """
    if 'SID' not in id:
        return []
    ...
    compounds = []
    id = id.replace('SID', '')
    url = 'https://pubchem.ncbi.nlm.nih.gov/rest/pug/substance/sid/{}/synonyms/JSON'.format(id)  # NOQA
    r = requests.get(url, timeout=60)
    rsp = r.json()
    if 'InformationList' in rsp:
        informationList = r.json()['InformationList']
...
def normalize_biothings(name):
    ...
    try:
        ...
        for name_part in name_parts:
            ...
            if name_part.startswith('CID'):
                url = 'http://c.biothings.io/v1/query?q=pubchem.cid:{}&{}'.format(name_part, fields)  # NOQA                
            elif name_part.startswith('CHEMBL'):
                url = 'http://c.biothings.io/v1/query?q=chembl.molecule_chembl_id:{}&{}'.format(name_part, fields)  # NOQA     
            elif name_part.startswith('CHEBI'):
                name_part = name_part.replace('CHEBI','CHEBI:')
                url = 'http://c.biothings.io/v1/chem/{}?{}'.format(name_part, fields)  # NOQA     
            else:    
                url = 'http://c.biothings.io/v1/query?q=chembl.pref_name:{}&{}'.format(name_part, fields)  # NOQA
            logging.debug(url)
            r = requests.get(url, timeout=60)
            rsp = r.json()
            if 'hits' not in rsp and 'chembl' in rsp:
                hits = [rsp]
            else:    
                hits = rsp.get('hits',[])
            if len(hits) == 0:
            ...
            hits = sorted(hits, key=lambda k: k.get('_score', 0), reverse=True)
            hit = hits[0]
            ...
            if hit.get('_score', 10) < 8.8:
                logging.debug(
                    'discarded, score too low {}'.format(hit['_score'])
                    )
                continue

            if 'chembl' not in hit:
                return []
            chembl = hit['chembl']
            synonym_fda = synonym_usan = synonym_inn = synonym_other = None
            if 'molecule_synonyms' in chembl:
                ...
                if type(molecule_synonyms) is list:
                    for molecule_synonym in molecule_synonyms:
                        ...
                        if molecule_synonym['syn_type'] == 'OTHER':
                            synonym_other = molecule_synonym['synonyms'].encode('utf8')
                else:
                    ...
                    if molecule_synonyms['syn_type'] == 'OTHER':
                        synonym_other = molecule_synonyms['synonyms'].encode('utf8')


            # must have a name            
            if not (synonym_fda or synonym_usan or synonym_inn or synonym_other):
                return []
            ...
            compound = {'ontology_term': ontology_term,
                        ...
                        synonym_inn or synonym_other or name_part,
                        'source': source
                        }
...
def normalize(name):
    ...
    if len(drugs) == 0 and name.startswith('CID'):
        # print 'normalize_pubchem_id', name
        drugs = normalize_pubchem_id(name)
    
    if len(drugs) == 0 and name.startswith('SID'):
        # print 'normalize_pubchem_substance_id', name
        drugs = normalize_pubchem_substance_id(name)
