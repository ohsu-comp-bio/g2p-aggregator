...
def test_responses(client):
    ...
    """
    rsp = client.get('/v1/beacon/query?referenceName=2&start=29436859&startMin=1&startMax=1&referenceBases=A&alternateBases=C&assemblyId=GRCh37')  # noqa
    rsp = json.loads(rsp.get_data())
    assert len(rsp) > 0
