...
def construct_hgvs(feature, complement=False, description=False, exclude={}):
    ...
    if pa:
        ...
        hgvs_type = 'g'
        if '37' in feature.get('referenceName', ''):
...
def _get_hgvs_set(clinvar_alleles, attempt=0):
    ...
    for variant_id in variant_ids:
        ...
        hgvs_set.update(
            set([x.text for x in hgvs_list if x.text])
        )
        variant_hgvs[variant_id] = hgvs_set
    return hgvs_set
...
def _test_feature_association(feature_association, expected_hgvs=''):
    ...
    for feature in feature_association['features']:
        if suffix in feature.get('hgvs_p_suffix', []):
            print "hgvs p suffix found"
            return
        elif suffix in feature.get('hgvs_g_suffix', []):
...
if __name__ == '__main__':
    ...
    jax_entry = {
        "features":[{"geneSymbol":"ERBB2","name":"Y772_A775dup","protein_allele":True}],
        "jax":{
